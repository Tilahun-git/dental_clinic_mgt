...
function LoginPageContent() {
  ...
  return (
    <div className="w-full max-w-sm">
      <div className="bg-[var(--bg-card)] rounded-3xl shadow-xl overflow-hidden border border-[var(--border)]">
        {/* Brand header */}
        <div className="bg-[#17231d] px-8 py-10 text-center">
          ...
          <p className="text-emerald-100 text-sm mt-1">Dental Clinic Management</p>
        </div>

        <div className="p-7">
          <form onSubmit={submitCredentials} className="mb-6 space-y-3">
            <div>
              <label className="block text-xs font-bold text-[var(--text-muted)] uppercase tracking-wide mb-1">Email</label>
              <input type="email" required value={email} onChange={event => { setEmail(event.target.value); setError(''); }} placeholder="you@smilecare.et"
                className="w-full border border-[var(--border)] bg-[var(--surface-soft)] text-[var(--text-base)] rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-600" />
            </div>
            <div>
              <label className="block text-xs font-bold text-[var(--text-muted)] uppercase tracking-wide mb-1">Password</label>
          ...
          <div className="mb-2">
            <div className="flex items-center gap-2 mb-4 border-t border-[#d8e4dc] pt-5">
              <p className="text-sm font-bold text-[var(--text-heading)]">Demo accounts</p>
            </div>
            <div className="grid grid-cols-2 gap-2">
              {demoUsers.map(u => (
                <button key={u.id} onClick={() => go(u.id)}
                  className="bg-[var(--surface-soft)] hover:bg-[var(--primary-lt)] text-emerald-700 dark:text-emerald-300 border border-[var(--border)] text-xs font-bold py-3 px-3 rounded-xl transition-all text-left">
                  <div className="font-bold text-sm">{roleLabel[u.role]}</div>
                  <div className="text-[var(--text-muted)] font-normal truncate mt-0.5 text-xs">{u.name}</div>
                </button>
              ))}
            </div>
          </div>

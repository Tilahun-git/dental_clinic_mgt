        ...
        return (
          <div key={category} className="mb-12">
            <div className="flex items-center gap-3 mb-6">
              ...
              <h2 className="text-xl font-bold text-[var(--text-heading)]">{category}</h2>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
              {categoryServices.map((service) => (
                <div
                  key={service.id}
                  className="bg-[var(--bg-card)] rounded-2xl border border-[var(--border)] overflow-hidden hover:shadow-md transition-shadow"
                >
                  <div className="h-40 bg-cover bg-center" style={{ backgroundImage: `url(${service.image})` }} role="img" aria-label={`${service.name} service`} />
                  <div className="p-6">
                  ...
                  <p className="text-[var(--text-muted)] text-sm mt-2 leading-relaxed">{service.description}</p>

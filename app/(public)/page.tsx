...
export default function HomePage() {
  ...
  return (
    <div className="page-bg">
      {/* Hero */}
      <section className="relative bg-[var(--surface-soft)] text-[var(--text-base)] overflow-hidden border-b border-[var(--border)]">
        <div className="absolute right-0 top-0 h-full w-2/5 bg-emerald-500/10" />
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24 relative">
          <div className="max-w-2xl">
            <div className="inline-flex items-center gap-2 text-emerald-600 text-sm font-bold uppercase tracking-[0.18em] mb-6">
              <span className="h-px w-8 bg-amber-500" />
              <span>Dental care, thoughtfully delivered</span>
            </div>
            <h1 className="text-5xl md:text-6xl font-extrabold leading-tight mb-4 text-[var(--text-heading)]">
              Trusted care,<br />
              <span className="text-emerald-600">made personal.</span>
            </h1>
            <p className="text-xl text-[var(--text-muted)] mb-8 leading-relaxed max-w-xl">
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <Link
                href="/services"
                className="inline-flex items-center gap-2 border border-[var(--border)] hover:bg-[var(--bg-card)] text-emerald-600 px-6 py-3 rounded-xl font-bold text-base transition-colors"
              >
                View Services
              </Link>
            </div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {previewServices.map((service) => (
            <div key={service.id} className="bg-[var(--bg-card)] rounded-2xl border border-[var(--border)] overflow-hidden hover:shadow-lg transition-shadow group">
              <div className="h-32 bg-cover bg-center" style={{ backgroundImage: `url(${service.image})` }} role="img" aria-label={`${service.name} service`} />
              <div className="p-6">
              <div className="text-4xl mb-3 grayscale">{service.icon}</div>
              <h3 className="font-semibold text-[var(--text-heading)] text-lg">{service.name}</h3>
              <p className="text-[var(--text-muted)] text-sm mt-1 mb-4">{service.description}</p>
              <div className="flex items-center justify-between">
                <span className="text-emerald-600 font-bold">from {service.price.toLocaleString()} ETB</span>
                <span className="text-[var(--text-muted)] text-xs">{service.duration} min</span>
              </div>
              </div>
            </div>
          ))}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {previewDentists.map((dentist) => (
              <div key={dentist.id} className="bg-[var(--bg-card)] rounded-2xl border border-[var(--border)] p-6 text-center hover:shadow-lg transition-shadow">
                <div className="w-20 h-20 bg-cover bg-center rounded-full ring-2 ring-amber-400 flex items-center justify-center text-white text-2xl font-bold mx-auto mb-4" style={{ backgroundImage: `url(${dentist.image})` }} role="img" aria-label={`${dentist.name} portrait`}>
                  <span className="sr-only">{dentist.avatar}</span>
                </div>
                <h3 className="font-bold text-[var(--text-heading)] text-lg">{dentist.name}</h3>
                <p className="text-amber-500 text-sm font-medium mt-1">{dentist.specialization}</p>
                <p className="text-[var(--text-muted)] text-sm mt-2">{dentist.experience} years experience</p>
                <p className="text-[var(--text-muted)] text-sm mt-3 line-clamp-2">{dentist.bio}</p>
              </div>
            ))}
          </div>
          <div className="text-center mt-8">

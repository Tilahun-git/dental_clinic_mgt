...
export function PublicNavbar() {
  ...
  return (
    <header className="sticky top-0 z-50 shadow-sm" style={{ background: 'var(--navbar)', borderBottom: '1px solid var(--border)' }}>
      <nav className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          <Link href="/" className="flex items-center gap-2">
            ...
            <span className="font-black text-[var(--text-heading)] text-xl tracking-tight">SENAKO</span>
          </Link>
            ...
            {navLinks.map(link => (
              <Link key={link.href} href={link.href}
                className={cn('px-3 py-2 rounded-xl text-sm font-semibold transition-colors',
                  pathname === link.href ? 'bg-[var(--primary-lt)] text-[var(--text-heading)]' : 'text-[var(--text-base)]/75 hover:bg-[var(--primary-lt)] hover:text-[var(--text-heading)]')}>
                {link.label}
              </Link>
            ))}
          </div>
          ...
          <div className="hidden md:flex items-center gap-2">
            <ThemeToggle />
            <Link href="/login" className="px-4 py-2 text-sm font-semibold text-[var(--text-base)] hover:bg-[var(--primary-lt)] rounded-xl transition-colors">
              Login
            </Link>
          ...
          <button className="md:hidden p-2 rounded-xl text-[var(--text-base)] hover:bg-[var(--primary-lt)]" onClick={() => setMobileOpen(!mobileOpen)}>
            ...
            {navLinks.map(link => (
              <Link key={link.href} href={link.href}
                className={cn('block px-3 py-2 rounded-xl text-sm font-semibold',
                  pathname === link.href ? 'bg-[var(--primary-lt)] text-[var(--text-heading)]' : 'text-[var(--text-base)]/75 hover:bg-[var(--primary-lt)]')}
